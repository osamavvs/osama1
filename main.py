...
def send_welcome(message: Message):
    ...
    if not check_subscription(message.from_user.id):
        ...
        bot.reply_to(message, "❌ أهلاً بك! يجب عليك الاشتراك في القناة أولاً:", reply_markup=markup)
        return
...
def handle_message(message: Message):
    ...
    try:
        # المحاولة عبر الرابط
        response = requests.post('https://test.socialfruit.co/api/gateway', headers=headers, json=json_data, timeout=15)
        
        if "success" in response.text.lower():
            bot.edit_message_text("✅ تم بنجاح إضافة التفاعلات", chat_id=message.chat.id, message_id=waiting_msg.message_id)
        else:
            bot.edit_message_text("❌ فشلت العملية (الموقع مضغوط حالياً).", chat_id=message.chat.id, message_id=waiting_msg.message_id)
                
    except Exception:
        bot.edit_message_text("❌ حدث خطأ في الاتصال بالسيرفر.", chat_id=message.chat.id, message_id=waiting_msg.message_id)
