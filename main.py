...
CHANNEL_ID = "@BBABB9" 
# قاموس لتخزين وقت آخر عملية لكل مستخدم
user_last_usage = {}
# قائمة سيرفرات متنوعة لزيادة استقرار الرشق
...
def handle_message(message: Message):
    ...
    # نظام الانتظار 5 دقائق
    user_id = message.from_user.id
    current_time = time.time()
    if user_id in user_last_usage and (current_time - user_last_usage[user_id] < 300):
        remaining = int(300 - (current_time - user_last_usage[user_id]))
        bot.reply_to(message, f"⚠️ يرجى الانتظار، يمكنك الرشق مرة أخرى بعد {remaining // 60} دقائق و {remaining % 60} ثوانٍ.")
        return
    ...
    try:
        ...
        waiting_msg = bot.reply_to(message, "⏳ جاري معالجة طلبك عبر سيرفر عشوائي...")
        ...
        if "success" in response.text.lower():
            user_last_usage[user_id] = time.time() # تسجيل وقت العملية
